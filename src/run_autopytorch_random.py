...
from autopytorch_utils import get_preprocessed_openml_dataset, data_to_train_test_valid
...
def run_on_dataset(cocktails, args, seed, budget, config):
    ...
    (
        ...
        y_valid,
        y_test
         ) = data_to_train_test_valid(X, y, config, seed)
    ...
    logger = get_named_client_logger(
        name=f"AutoPyTorch:{dataset_openml.name}:{seed}",
        port=logger_port)
    ...
    if args.max_configs > 1:
        sampled_configurations = configuration_space.sample_configuration(
            args.max_configs - 1)
        configurations += sampled_configurations \
            if isinstance(sampled_configurations, list) else [sampled_configurations]
    ...
    try:
        ...
        sorted_run_history = sorted(
            run_history,
            key=lambda x: run_history[x]['cost']['val'],
            reverse=True
        )
...
if __name__ == '__main__':
    ...
    final_result = run_on_dataset(cocktails, args, seed, budget, run_config)
