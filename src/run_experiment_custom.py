...
from autopytorch_utils import data_to_train_test_valid, get_preprocessed_openml_dataset
...
def train_model_on_config(config=None):
    ...
    with wandb.init(config=config) as run:
        ...
        run.config.update(preprocessing_config)
        config = wandb.config
        print(config)
        ...
        try:
            ...
            for i in range(n_iter):
                ...
                print(rng.randn(1))
                t = time.time()
                categorical_indicator, x_train, x_val, x_test, y_train, y_val, y_test = generate_dataset(config, rng)
                data_generation_time = time.time() - t
                print("Data generation time:", data_generation_time)
                # print(y_train)
                print(x_train.shape)

                # Update shape of data according to model type
                x_train, x_val, x_test, y_train, y_val, y_test = update_data_shapes(config, x_train, x_val, x_test, y_train, y_val, y_test)
                ...
                if config["model_type"] == "skorch":
                    log_model_history_skorch(config, model)
            ...
            if config["model_type"] == "tab_survey":
                ...
                print("Removing ")
                model_checkpoint_path = WORKING_DIR / "output" / "saint" / str(config["data__keyword"]) / "tmp" / "m_{}_best.pt".format(model_id)
...
def update_data_shapes(config, x_train, x_val, x_test, y_train, y_val, y_test):
...
def generate_dataset(config, rng):
    kwargs = {k.replace("data__", ""): v for k, v in config.items() if k.startswith("data__")}
    kwargs['dataset_id'] = kwargs.pop('keyword')
    X, y, categorical_indicator = get_preprocessed_openml_dataset(**kwargs)
    kws = ["train_prop", "val_test_prop", "max_val_samples", "max_train_samples", "max_test_samples"]
    kwargs = {k: v for k, v in config.items() if k in kws}
    x_train, x_val, x_test, y_train, y_val, y_test = data_to_train_test_valid(X, y, config=kwargs, rng=rng)
    return categorical_indicator,x_train,x_val,x_test,y_train,y_val,y_test

def log_metrics(train_scores, val_scores, test_scores, r2_train_scores, r2_val_scores, r2_test_scores, times, n_iter, x_train, x_test, data_generation_time, start_time, r2_train, r2_val, r2_test, train_score, val_score, test_score, end_time, processor):
    if n_iter > 1:
...
def log_model_history_skorch(config, model):
    if config["regression"]:
