...
def get_updates_for_regularization_cocktails(categorical_indicator: list[bool] = None):
    """
    ...
    print(f"In updates :{categorical_indicator}")
    if categorical_indicator is not None:
        ...
        # to maintain backwards compatibility (previously we were running with assuming only numerical)
...
def run_on_autopytorch(
    ...
    if not skip_training:
        ...
        search_space_updates, include_updates = get_updates_for_regularization_cocktails(
            categorical_indicator=categorical_indicator) # if cocktails else get_updates_for_autopytorch_tabular()
